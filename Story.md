...
- Mappato il flusso dati tra i 3 progetti:
  - **Progetto A** (ep-v1-gestionale): Gestionale - sistema core
  - **Progetto B** (ep-iscrizioni-public-main): Pagina Pubblica - landing page iscrizioni
  - **Progetto C** (ep-portal): Enrollment Portal - completamento iscrizione

### Flusso Dati
1. Utente compila form in Project B → `raw_registrations` / `registrations`
2. Cloud Functions triggera → invia a Project A (`receiveLeadV2`)
3. Lead salvato in `incoming_leads` (Project A)
4. Utente clicca link WhatsApp → `enrollmentGateway` → reindirizza a Project C
5. Project C chiama `getEnrollmentData` → recupera dati lead, abbonamenti, sedi
6. Utente completa iscrizione → `processEnrollment` → crea enrollment + lezioni

### Fix Implementati in Project A
1. **Controllo Idempotenza:** Previene duplicazione lead basato su `originalId` o `email + childName`
2. **Validazione Input:** Email e childName sono ora obbligatori

### Project C (ep-portal)
- Nessuna modifica diretta - il codice è parte del frontend Vercel
- Le Cloud Functions `getEnrollmentData` e `processEnrollment` sono deployate in Project A
